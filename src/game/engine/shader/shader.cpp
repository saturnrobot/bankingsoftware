...
namespace game
{
    ...
    void Shader::SetUniform1i(const GLchar *name, int value) { glUniform1i(glGetUniformLocation(shader_program_, name), value); }

    void Shader::SetUniform1f(const GLchar *name, float value) { glUniform1f(glGetUniformLocation(shader_program_, name), value); }

    void Shader::SetUniform2f(const GLchar *name, const glm::vec2 &vector) { glUniform2f(glGetUniformLocation(shader_program_, name), vector.x, vector.y); }

    void Shader::SetUniform3f(const GLchar *name, const glm::vec3 &vector) { glUniform3f(glGetUniformLocation(shader_program_, name), vector.x, vector.y, vector.z); }

    void Shader::SetUniform4f(const GLchar *name, const glm::vec4 &vector) { glUniform4f(glGetUniformLocation(shader_program_, name), vector.x, vector.y, vector.z, vector.w); }

    void Shader::SetUniformMat4(const GLchar *name, const glm::mat4 &matrix) { glUniformMatrix4fv(glGetUniformLocation(shader_program_, name), 1, GL_FALSE, glm::value_ptr(matrix)); }

    Shader::~Shader() { glDeleteProgram(shader_program_); }

    void Shader::Enable() { glUseProgram(shader_program_); }
